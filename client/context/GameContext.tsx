...
export type GameState = {
  ...
  openPack: (
    pack: PackType,
  ) => { cards: OpenedCard[]; packPrice: number; gainedCoins: number } | null;
...
export const GameProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const [coins, setCoins] = usePersistentState<number>(
    `${STORAGE_KEY}:coins`,
    2000,
  );
  ...
  const [inventory, setInventory] = usePersistentState<Record<string, Player>>(
    `${STORAGE_KEY}:inv`,
    {},
  );
  const [counts, setCounts] = usePersistentState<Record<string, number>>(
    `${STORAGE_KEY}:counts`,
    {},
  );

  const playersByTier = useMemo(() => {
    const map: Record<Tier, Player[]> = {
      Common: [],
      Rare: [],
      Epic: [],
      Legend: [],
