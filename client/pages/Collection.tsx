...
export default function Collection() {
  ...
  const filtered = players.filter((p) => {
    ...
    if (q && !(`${p.name} ${p.team} ${p.role}`.toLowerCase().includes(q.toLowerCase()))) return false;
    return true;
  });
    ...
    <div className="min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#0b1220] via-[#0b1220] to-[#020617] text-white">
      ...
      <div className="container mx-auto px-4 py-10">
        ...
        <div className="mt-6 grid grid-cols-1 sm:grid-cols-4 gap-3">
          <Input placeholder="Search by name, team..." value={q} onChange={(e) => setQ(e.target.value)} className="bg-white/5 border-white/10" />
          <Select onValueChange={(v) => setTier(v)}>
            <SelectTrigger className="bg-white/5 border-white/10"><SelectValue placeholder="Tier: All" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="All">All</SelectItem>
              {(["Common","Rare","Epic","Legend"] as Tier[]).map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}
            </SelectContent>
          </Select>
          <Select onValueChange={(v) => setRole(v)}>
            <SelectTrigger className="bg-white/5 border-white/10"><SelectValue placeholder="Role: All" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="All">All</SelectItem>
              {(["BAT","BOWL","AR","WK"] as Role[]).map((r) => <SelectItem key={r} value={r}>{r}</SelectItem>)}
            </SelectContent>
          </Select>
          <Select onValueChange={(v) => setTeam(v)}>
            <SelectTrigger className="bg-white/5 border-white/10"><SelectValue placeholder="Team: All" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="All">All</SelectItem>
        ...
        <div className="mt-8 grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-4">
          {filtered.length === 0 && (
            <div className="col-span-full text-center text-white/70">No cards yet. Open some packs!</div>
          )}
          {filtered.map((p) => (
